...
class TestFMatrix(unittest.TestCase):
    ...
    def test_get_theano_dataset(self):
        trainset_filename = resource_filename(
            "patent_parsing_tools.utils.tests", "train.txt"
        )
        dictionary_filename = resource_filename(
            "patent_parsing_tools.utils.tests", "dictionary.txt"
        )
        train_set_x = fmatrix.get_theano_dataset(
            trainset_filename, dictionary_filename
        )
        self.assertEqual(
            train_set_x.get_value(borrow=True).shape, (10000, 10000)
        )
    ...
    def test_load_patents_dataset(self):
        ...
        for k in dataset.keys():
            if i < 100:
                dataset2[k] = [
                    dataset[k][0][:25],
                    np.random.choice(["abc", "def", "gh"], 2).tolist(),
                ]
                i += 1
            else:
                break
    ...
    def test_load_patents_dataset2(self):
        test_set_dir = resource_filename(
            "patent_parsing_tools.utils.tests", "test_set"
        )
    ...
    def test_concat_categories(self):
        ...
        self.assertEqual(
            fmatrix.concat([], categories_list), ["a", "a44", "a44F", "a44F54"]
        )

    def test_parse_categories(self):
        categories_list = [["a", "44", "F", "54"], ["b", "11", "Z", "53"]]
        self.assertEqual(
            fmatrix.parse_categories(categories_list, 0), ["a", "b"]
        )
        self.assertEqual(
            fmatrix.parse_categories(categories_list, 1), ["a44", "b11"]
        )
